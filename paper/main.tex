...
Our ablation confirms this empirically: replacing the semantic $C$ with an identity-like cost (\ie, $C_{ij}=\mathbf{1}[i\neq j]$) collapses \jko\ to KL performance at 0.713 nDCG@10 on SciFact — the semantic geometry is what drives the qualitative difference.
...
\Cref{tab:main} shows nDCG@10 with 95\% CIs. \jko\ achieves the best result on all five datasets under held-out hyperparameter tuning. Paired bootstrap confirms significance vs.\ the cross-encoder on four datasets (95\% CI excludes zero).

\begin{table}[t]
\centering\small
\caption{nDCG@10 with 95\% bootstrap CI. \textbf{Bold} = best. $\dagger$ = 95\% CI of diff vs Cross-Enc excludes 0.}
\label{tab:main}
...
Iter-RetGen & 0.677 & ---   & ---  & ---   & ---  \\
KL-Prox     & 0.712 & 0.357 & 0.717 & 0.413 & 0.197 \\
\midrule
\jko\ (ours)  & \textbf{0.713}$^\dagger$ & \textbf{0.359}$^\dagger$ & \textbf{0.725}$^\dagger$ & \textbf{0.411}$^\dagger$ & \textbf{0.196} \\
...
\begin{table}[h]
\centering\small
\caption{Mean $\protect\mWdist$ under paraphrase (lower = more stable).}
\label{tab:stability}
\begin{tabular}{@{}lcccc@{}}
\toprule
...
\begin{table}[h]
\centering\small
\caption{Distractor leakage (lower = better). SciFact, n=150 queries.}
...
\begin{table}[h]
\centering\small
\caption{Ablation on SciFact test. All changes are single-component relative to the full \jko\ method.}
\label{tab:ablation}
...
\midrule
Full \jko\ (tuned) & --- & 0.713 & --- \\
KL-proximal   & $W^2\to\mathrm{KL}$ & 0.713 & $+0.000$ \\
No proximal   & drop prox.\ term & 0.710 & $-0.003$ \\
Identity $C$  & $C_{ij}=\mathbf{1}[i\!\neq\!j]$ & 0.713 & $+0.000$ \\
Random $C$    & $C_{ij}\sim\mathrm{Unif}[0,4]$ & 0.707 & $-0.006$ \\
No entropy ($\lambda{=}0$) & drop entropy & 0.679 & $-0.034^*$ \\
No redundancy ($\rho{=}0$) & drop redund. & 0.694 & $-0.019$ \\
One JKO step ($T{=}1$)    & fewer iters & 0.699 & $-0.014$ \\
\bottomrule
\multicolumn{4}{l}{\small $^*$ 95\% CI excludes 0.}
\end{tabular}
\end{table}
