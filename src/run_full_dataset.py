...
def main():
    ...
    p.add_argument("--index-root", default=None,
                   help="Override index root dir (default: <project>/indices)")
    args = p.parse_args()

    if args.index_root:
        INDEX_ROOT = Path(args.index_root).resolve()

    print(f"=== {args.dataset}/{args.split} (index: {INDEX_ROOT}) ===")
    ...
    q_emb_all = np.load(INDEX_ROOT / args.dataset / f"q_embeddings_{args.split}.npy")  # uses current INDEX_ROOT
